...
function coerceImageUrls(x: any): string[] {
  ...
  return Array.from(new Set(list.filter((u) => typeof u === "string" && u.trim())));
}
...
export default function ProdutosPage() {
  ...
  function normalizeApiItem(x: any): Prod {
    const id =
      x?.id || x?.docId || x?._id || x?.refId || x?.sku ||
      (typeof crypto !== "undefined" ? crypto.randomUUID() : `${x?.sku}-${Math.random()}`);
  ...
  const visibleItems = useMemo(() => {
    ...
    const filtered = (items ?? []).filter((p) => {
      const passQuery = !q || p.name.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q);
    ...
    <main className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link href="/vendedor/produtos/importar-csv" className="rounded-lg border px-4 py-2.5 text-sm">
            Importar CSV
          </Link>
          ...
          <button onClick={loadProducts} className="rounded-lg border px-3 py-2 text-sm" title="Recarregar">
            Recarregar
          </button>
        </div>
      </div>
              ...
              <li key={p.id} className="flex items-center justify-between gap-3 px-4 py-4">
                <div className="flex items-center gap-3">
                  <div className="h-14 w-14 rounded-lg border bg-white overflow-hidden flex items-center justify-center">
                    {thumb ? (
                      <img src={thumb} alt={p.name} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                    ) : (
                  ...
                  <div>
                    ...
                    <div className="text-sm opacity-70">
                      R{"$"}
                      {Number(p.price).toLocaleString("pt-BR", { minimumFractionDigits: 2 })} • Estoque: {p.stock ?? 0} •{" "}
                ...
                <div className="flex gap-2">
                  <Link href={`/vendedor/produtos/${encodeURIComponent(p.id)}`} className="rounded-lg border px-3 py-2 text-sm">
                    Editar
                  </Link>
                </div>
              </li>
